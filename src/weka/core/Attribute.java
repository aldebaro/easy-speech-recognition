...
public class Attribute implements Copyable, Serializable {
	...
	public final String toString() {
		...
		if (isNominal()) {
			text.append('{');
			Enumeration enume = enumerateValues();
	...
	/**
	 * Returns a description of this attribute in C4.5 format. Quotes
	 * strings if they contain whitespace characters, or if they
	 * are a question mark.
	 *
	 * @return a description of this attribute as a string
	 */
	public final String toStringAsInC45() {

		StringBuffer text = new StringBuffer();

		text.append(Utils.quote(m_Name) + ": ");
		if (isNominal()) {
			//text.append('{');
			Enumeration enume = enumerateValues();
			while (enume.hasMoreElements()) {
	...
	final void delete(int index) {
		...
		if (!isNominal() && !isString())
			...
			Enumeration enume = m_Hashtable.keys();
			while (enume.hasMoreElements()) {
	Object string = enume.nextElement();
